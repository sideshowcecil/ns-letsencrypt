...
class appfwprofile_jsonxssurl_binding(base_resource) :
	""" Binding class showing the jsonxssurl that can be bound to appfwprofile.
	"""
	def __init__(self) :
		self._jsonxssurl = None
		self._state = None
		self._comment = None
	...
	@property
	def jsonxssurl(self) :
		r"""A regular expression that designates a URL on the Json XSS URL list for which XSS violations are relaxed.
		Enclose URLs in double quotes to ensure preservation of any embedded spaces or non-alphanumeric characters.<br/>Minimum length =  1<br/>Maximum length =  2047.
		"""
		try :
			return self._jsonxssurl
		except Exception as e:
			raise e

	@jsonxssurl.setter
	def jsonxssurl(self, jsonxssurl) :
		r"""A regular expression that designates a URL on the Json XSS URL list for which XSS violations are relaxed.
		Enclose URLs in double quotes to ensure preservation of any embedded spaces or non-alphanumeric characters.<br/>Minimum length =  1<br/>Maximum length =  2047
		"""
		try :
			self._jsonxssurl = jsonxssurl
		except Exception as e:
			raise e
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(appfwprofile_jsonxssurl_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.appfwprofile_jsonxssurl_binding
		except Exception as e :
			raise e
	...
	def filter_add_parameters(cls, resource) :
		...
		"""
		addresource = appfwprofile_jsonxssurl_binding()
		addresource.name = resource.name
		addresource.comment = resource.comment
		addresource.state = resource.state
		addresource.jsonxssurl = resource.jsonxssurl
		...
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					updateresources = [appfwprofile_jsonxssurl_binding() for _ in range(len(resource))]
	...
	def filter_delete_parameters(cls, resource) :
		...
		"""
		deleteresource = appfwprofile_jsonxssurl_binding()
		deleteresource.name = resource.name
		deleteresource.jsonxssurl = resource.jsonxssurl
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					deleteresources = [appfwprofile_jsonxssurl_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch appfwprofile_jsonxssurl_binding resources.
		"""
		try :
			if not name :
				obj = appfwprofile_jsonxssurl_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of appfwprofile_jsonxssurl_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count appfwprofile_jsonxssurl_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwprofile_jsonxssurl_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of appfwprofile_jsonxssurl_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwprofile_jsonxssurl_binding()
...
class appfwprofile_jsonxssurl_binding_response(base_response) :
	def __init__(self, length=1) :
		self.appfwprofile_jsonxssurl_binding = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.appfwprofile_jsonxssurl_binding = [appfwprofile_jsonxssurl_binding() for _ in range(length)]
