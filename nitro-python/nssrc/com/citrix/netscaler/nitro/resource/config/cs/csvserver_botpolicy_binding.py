...
class csvserver_botpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(csvserver_botpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.csvserver_botpolicy_binding
		except Exception as e :
			raise e
	...
	def filter_add_parameters(cls, resource) :
		...
		"""
		addresource = csvserver_botpolicy_binding()
		...
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					updateresources = [csvserver_botpolicy_binding() for _ in range(len(resource))]
	...
	def filter_delete_parameters(cls, resource) :
		...
		"""
		deleteresource = csvserver_botpolicy_binding()
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					deleteresources = [csvserver_botpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch csvserver_botpolicy_binding resources.
		"""
		try :
			if not name :
				obj = csvserver_botpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of csvserver_botpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count csvserver_botpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = csvserver_botpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of csvserver_botpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = csvserver_botpolicy_binding()
...
class csvserver_botpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		self.csvserver_botpolicy_binding = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.csvserver_botpolicy_binding = [csvserver_botpolicy_binding() for _ in range(length)]
