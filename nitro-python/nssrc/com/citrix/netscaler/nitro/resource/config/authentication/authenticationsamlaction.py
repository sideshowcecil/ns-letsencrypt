...
class authenticationsamlaction(base_resource) :
	...
	def metadataurl(self) :
		r"""This URL is used for obtaining saml metadata. Note that it fills samlIdPCertName and samlredirectUrl fields so those fields should not be updated when metadataUrl present.
		"""
		try :
	...
	def samlidpcertname(self) :
		r"""Name of the SSL certificate used to verify responses from SAML Identity Provider (IdP). Note that if metadateURL is present then this filed should be empty.<br/>Minimum length =  1.
		"""
		try :
	...
	def samlredirecturl(self) :
		r"""URL to which users are redirected for authentication. Note that if metadateURL is present then this filed should be empty.<br/>Minimum length =  1.
		"""
		try :
