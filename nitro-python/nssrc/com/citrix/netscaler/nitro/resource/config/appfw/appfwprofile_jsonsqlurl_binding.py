...
class appfwprofile_jsonsqlurl_binding(base_resource) :
	""" Binding class showing the jsonsqlurl that can be bound to appfwprofile.
	"""
	def __init__(self) :
		self._jsonsqlurl = None
		self._state = None
		self._comment = None
	...
	@property
	def jsonsqlurl(self) :
		r"""A regular expression that designates a URL on the Json SQL URL list for which SQL violations are relaxed.
		...
		"""
		try :
			return self._jsonsqlurl
		except Exception as e:
			raise e

	@jsonsqlurl.setter
	def jsonsqlurl(self, jsonsqlurl) :
		r"""A regular expression that designates a URL on the Json SQL URL list for which SQL violations are relaxed.
		Enclose URLs in double quotes to ensure preservation of any embedded spaces or non-alphanumeric characters.<br/>Minimum length =  1<br/>Maximum length =  2047
		"""
		try :
			self._jsonsqlurl = jsonsqlurl
		except Exception as e:
			raise e
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(appfwprofile_jsonsqlurl_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.appfwprofile_jsonsqlurl_binding
		except Exception as e :
			raise e
	...
	def filter_add_parameters(cls, resource) :
		...
		"""
		addresource = appfwprofile_jsonsqlurl_binding()
		addresource.name = resource.name
		addresource.comment = resource.comment
		addresource.state = resource.state
		addresource.jsonsqlurl = resource.jsonsqlurl
		...
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					updateresources = [appfwprofile_jsonsqlurl_binding() for _ in range(len(resource))]
	...
	def filter_delete_parameters(cls, resource) :
		...
		"""
		deleteresource = appfwprofile_jsonsqlurl_binding()
		deleteresource.name = resource.name
		deleteresource.jsonsqlurl = resource.jsonsqlurl
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					deleteresources = [appfwprofile_jsonsqlurl_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch appfwprofile_jsonsqlurl_binding resources.
		"""
		try :
			if not name :
				obj = appfwprofile_jsonsqlurl_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of appfwprofile_jsonsqlurl_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count appfwprofile_jsonsqlurl_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwprofile_jsonsqlurl_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of appfwprofile_jsonsqlurl_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwprofile_jsonsqlurl_binding()
...
class appfwprofile_jsonsqlurl_binding_response(base_response) :
	def __init__(self, length=1) :
		self.appfwprofile_jsonsqlurl_binding = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.appfwprofile_jsonsqlurl_binding = [appfwprofile_jsonsqlurl_binding() for _ in range(length)]
