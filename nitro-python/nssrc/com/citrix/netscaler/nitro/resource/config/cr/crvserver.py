...
class crvserver(base_resource) :
	...
	def useoriginipportforcache(self) :
		r"""Use origin ip/port while forwarding request to the cache. Change the destination IP, destination port of the request came to CR vserver to Origin IP and Origin Port and forward it to Cache.<br/>Default value: NO<br/>Possible values = YES, NO.
		"""
		try :
